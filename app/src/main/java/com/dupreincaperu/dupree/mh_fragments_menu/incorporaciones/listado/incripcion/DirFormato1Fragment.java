...
import com.dupreincaperu.dupree.databinding.FragmentDireccionFormato1Binding;
...
//                permissionImage();
    ...
    public boolean validate() {
        ...
        if(model.getComplemento().isEmpty()){
            valid.setLoginError(getString(R.string.campo_requerido),binding.txtComplemento);
            return false;
        } else if(model.getDirereferencia().isEmpty()){
            valid.setLoginError(getString(R.string.campo_requerido),binding.txtdirereferencia);
            return false;
        }
    ...
    public boolean validateDirResidencia(){
       ...
       /* if (model.getTipo_via1().isEmpty())
        ...
        if (model.getTipo_via1().toUpperCase().isEmpty())
        {
            msgToast("Campo tipo via requerido");
        ...
        dialog.loadData(title, data, itemSelected, new SingleListDialog.ListenerResponse() {
            ...
            public void result(ModelList item) {
                switch(id){
                    ...
                    case R.id.txt_tipo_via:
                        //binding.txtTipoVia.setError(null);
                        model.setTipo_via1(item.getName());
                        break;
                }
            }
